...
def get_user_scores_data(user : ClientUser , user_messages : List[Message] = [], limit : int = 1000):
  ...
  for msg in user_messages: 
    ...
    if (location is not None):
      ...
      sub_data = data.loc[data['player'] == player_submission_dict['player']]
      sub_data = sub_data.loc[sub_data['wordle_number'] == player_submission_dict['wordle_number']]
      entry_already_exists = bool(len(sub_data) != 0)

      
      # entry_already_exists = bool(data.loc[(data['player'] == player_submission_dict['player']) & (data['wordle_number'] == player_submission_dict['wordle_number'])]  is not None)
      # entry_already_exist = data.isin({'player':player_submission_dict['player'], 'wordle_number': player_submission_dict['wordle_number'] }) outdated
      ...
      if not (date_does_not_match_wordle_number or  (entry_already_exists)): # none of of these conditions should be True to include data
        ...
        # player_submission_data = pd.DataFrame(player_submission_dict)
